...
## Tâche 1 — Direction artistique alignée sur le logo
...
Objectif : faire évoluer index-13.html + style.css pour que la home respire la même identité que ce logo, sans repartir de zéro. Garde la structure existante (header sticky, hero, bandeau promo défilant, section #collection "mur des casquettes", section #about, section "3 étapes", section #contact, footer) et les variables CSS actuelles (--cream, --ink, --teal, --teal-deep, --gold, --red).

Change :
1. Typographie des titres : renforcer l'usage d'Anton pour les gros titres façon logo, et ajouter une touche "brush/graffiti" sur au moins un élément fort du hero (ex: le mot HALAWA ou un slogan) — en CSS pur (text-shadow, transform léger, halo de couleur), pas d'image supplémentaire lourde.
2. Ajouter une texture/ambiance "street" subtile en fond du hero et de la section contact (éclaboussures de peinture façon logo, en SVG ou CSS, très discrètes, sans nuire à la lisibilité).
3. Reprendre le motif "couronne" du logo comme petit élément graphique récurrent (ex: à côté de l'eyebrow, ou dans les steps).
4. Garder tout le JS de navigation, panier, checkout WhatsApp intact — ne touche pas à script.js dans cette tâche, seulement à index-13.html (structure/markup si besoin) et style.css.
5. Le rendu doit rester "simple" : pas de surcharge, on ajoute 2-3 signatures visuelles fortes, pas dix.

Ne casse aucune fonctionnalité existante (drawer panier, modal checkout, scroll reveal, responsive).
```
